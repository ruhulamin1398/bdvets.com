...
@section('content')
    ...
    <div class="container-fluid">
        <div class="card mb-4 shadow">
            ...
            <div class="card-header py-3 bg-abasas-dark">
                <nav class="navbar navbar-dark ">
                    <a class="navbar-brand"> Add Necropsy</a>

                </nav>
            </div>

            <div class="card-body">
                <form method="POST" action="{{ route('necropsies.store') }}">
                    @csrf
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-header py-3 bg-abasas-dark">
                <nav class="navbar navbar-dark ">
                    <a class="navbar-brand"> Necropsy List</a>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped table-bordered" id="dataTable1" width="100%" cellspacing="0">
                        ...
                        <?php $i = 1; ?>
                        @foreach ($necropsyes as $necropsy)
                            <?php $id = $necropsy->id; ?>
                            <tr class="data-row">
                                <td class="iteration">{{$i++}}</td>
                                <td class="  word-break name">{{$necropsy->name}}</td>
                                <td class=" word-break description ">{{$necropsy->description}}</td>


                                <td class="align-middle">
                                    <button type="button" class="btn btn-success" id="necropsy-edit-item" data-item-id={{$id}}> <i class="fa fa-edit" aria-hidden="false"> </i></button>


                                    <form method="POST" action="{{ route('necropsies.destroy',  $necropsy->id )}} " id="delete-form-{{ $necropsy->id }}" style="display:none; ">
                                        {{csrf_field() }}
                                        {{ method_field("delete") }}
                                    </form>
                                    ...
                                    <button onclick="if(confirm('are you sure to delete this')){
                                        document.getElementById('delete-form-{{ $necropsy->id }}').submit();
                                        }
                                        else{
    ...
    <!-- Attachment Modal -->
    <div class="modal fade" id="necropsy-edit-modal" tabindex="-1" role="dialog" aria-labelledby="edit-modal-label" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body" id="attachment-body-content">
                    <form id="necropsy-edit-form" class="form-horizontal" method="POST" action="">
                    @csrf
                        @method('put')
